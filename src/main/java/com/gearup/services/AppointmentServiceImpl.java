...
//	Get Appointments By Customer ID
	...
	public List<Appointment> getAppointmentsByCustomerId(Long customerId) {
		...
		Customer customer = customerRepo.findByUserDetails_Id(customerId)
...
//	Get Upcoming Appointments For Customer (Pending and Confirmed)
    ...
    public List<Appointment> getUpcomingAppointments(Long userId) {
        ...
        // FIXED: Find Customer by User ID first
        Customer customer = customerRepo.findByUserDetails_Id(userId)
                .orElseThrow(() -> new ResourceNotFoundException("Customer Details Not Found for User ID: " + userId));

        List<Status> statuses = Arrays.asList(Status.PENDING, Status.CONFIRMED);
        
        // Use the real Customer ID found above
        return appointmentRepo.findByCustomer_IdAndStatusIn(customer.getId(), statuses);
    }
...
//	Get History of Appointments For Customer (Completed and Cancelled)
    ...
    public List<Appointment> getAppointmentHistory(Long userId) {
        ...
        List<Status> statuses = Arrays.asList(Status.COMPLETED, Status.CANCELLED);
...
//	Book Appointment Details
	...
	public ApiResponse bookNewAppointment(AppointmentBookingDto appointmentDetails) {
		...
		Customer customer = customerRepo.findByUserDetails_Id(appointmentDetails.getCustomerId())
				.orElseThrow(() -> new ResourceNotFoundException(
						"Customer ID " + appointmentDetails.getCustomerId() + " Not Found"));
		TimeSlot timeSlot = timeSlotRepo.findById(appointmentDetails.getTimeSlotId())
				.orElseThrow(() -> new ResourceNotFoundException(
						"Time Slot ID " + appointmentDetails.getTimeSlotId() + " Not Found"));
		if (timeSlot.isBooked()) {
			throw new ResourceAlreadyExistsException(
					"Time Slot Already Booked for This Time Period, Try Different Time Slot");
		}
		...
		newPayment.setTransactionId(System.currentTimeMillis()); // Temporary Useless ID, Need to Replace with Gateway
																	// ID later
...
//	Mark The Appointment as Completed
	...
	public ApiResponse markAppointmentCompleted(Long appointmentId) {
		...
		Appointment existingAppointment = appointmentRepo.findById(appointmentId)
				.orElseThrow(() -> new ResourceNotFoundException("Appointment ID " + appointmentId + " Not Found"));

		if (existingAppointment.getStatus() != Status.CONFIRMED) {
			throw new RuntimeException(
					"Cannot Mark This Appointment as Completed. Current Status: " + existingAppointment.getStatus());
		}
...
//	Update Appointment Details
	...
	public ApiResponse updateAppointment(Long apptId, AppointmentBookingDto appointmentDetails) {
		...
		Appointment existingAppointment = appointmentRepo.findById(apptId)
				.orElseThrow(() -> new ResourceNotFoundException("Appointment ID " + apptId + " Not Found"));

		if (existingAppointment.getStatus() == Status.CANCELLED
				|| existingAppointment.getStatus() == Status.COMPLETED) {
		...
		if (!oldSlotId.equals(newSlotId)) {
			...
			TimeSlot newSlot = timeSlotRepo.findById(newSlotId)
					.orElseThrow(() -> new ResourceNotFoundException("New Time Slot ID " + newSlotId + " not found"));

			if (newSlot.isBooked()) {
				throw new ResourceAlreadyExistsException(
						"The Selected New Time Slot is Already Booked. Please Choose Another Time Slot");
			}
...
//	Update Appointment Status
	@Override
	public ApiResponse updateAppointmentStatus(Long apptId, @Valid AppointmentBookingDto appointmentDetails) {
...
//	Cancel Appointment
	...
	public ApiResponse cancelAppointment(Long apptId) {
		...
		Appointment existingAppt = appointmentRepo.findById(apptId)
				.orElseThrow(() -> new ResourceNotFoundException("Appointment ID " + apptId + " Not Found"));
